...
OpMonPublisher::OpMonPublisher(const nlohmann::json& conf)
{
  ...
  if (it == conf.end()) {
    throw MissingParameter(ERS_HERE, "bootstrap", nlohmann::to_string(conf));
  }

  k_conf->set("bootstrap.servers", *it, errstr);
  if (!errstr.empty()) {
    throw FailedConfiguration(ERS_HERE, "bootstrap.servers", errstr);
  }
  ...
  if (it != conf.end())
    m_default_topic = "monitoring." + it->get<std::string>();
}
...
void
OpMonPublisher::publish(dunedaq::opmon::OpMonEntry&& entry) const
{
