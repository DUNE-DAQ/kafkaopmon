...
JsonFlattener::JsonFlattener(const nlohmann::json& j)
{
  ...
  if (j[JSONTags::parent].size() != 1) {
    throw OpmonJSONValidationError(ERS_HERE,
                                   "Expected 1 top-level entry, found " + std::to_string(j[JSONTags::parent].size()));
  }
...
void
JsonFlattener::parse_json(std::string path, const nlohmann::json& j)
{
    ...
    if (obj.count(JSONTags::properties)) {
      ...
      for (auto& pstruct : obj[JSONTags::properties].items()) {
        ...
        if (pstruct.value().count(JSONTags::time) == 0 || pstruct.value().count(JSONTags::data) == 0)
          throw OpmonJSONValidationError(
            ERS_HERE, pstruct.key() + " has no " + JSONTags::time + " or " + JSONTags::data + " tag");
